...
class XGBOD_uKNN(BaseDetector):
    """
    XGBOD_uKNN is an unsupervised outlier detection algorithm that integrates the KNN
    ...
    def __init__(self, max_depth=3, learning_rate=0.1, n_estimators=100, objective="binary:logistic", booster='gbtree',
        ...
        """
        Initialize XGBOD_uKNN with hyperparameters for the XGBoost classifier.
        """
        super(XGBOD_uKNN, self).__init__()
    ...
    def _init_detectors(self, X):
        """
        Initialize the base anomaly detection models (e.g., KNN) for use with XGBOD.
        The models will be used to generate decision scores for training.
        """
        knn = KNN()  # KNN detector is used here
        detectors = [knn]
        standardization_flags = [True]  # Flag to apply standardization
        return detectors, standardization_flags

    def fit(self, X, y):
        """
        Fit the XGBOD_uKNN model to the training data X and labels y.
        """
        X, y = check_X_y(X, y)
        X = check_array(X)
    ...
    def _get_detector_scores(self, X):
        """
        Get the decision score from the KNN detector.
        """
