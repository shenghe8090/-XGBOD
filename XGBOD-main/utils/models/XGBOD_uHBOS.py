...
class XGBOD_uHBOS(BaseDetector):
    """
    XGBOD_uHBOS is an unsupervised outlier detection algorithm that integrates the HBOS
    ...
    def __init__(self, max_depth=3, learning_rate=0.1, n_estimators=100, objective="binary:logistic", booster='gbtree',
        ...
        """
        Initialize XGBOD_uHBOS with hyperparameters for the XGBoost classifier.
        """
    ...
    def _init_detectors(self, X):
        """
        Initialize the HBOS anomaly detector.
        """
        hbos = HBOS()
        detectors = [hbos]
    ...
    def fit(self, X, y):
        """
        Fit the XGBOD_uHBOS model to the training data X and labels y.
        """
        X, y = check_X_y(X, y)
        X = check_array(X)
    ...
    def _get_detector_scores(self, X):
        """
        Get the decision score from the HBOS detector.
        """
