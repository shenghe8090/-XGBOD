...
class XGBOD_uIForest(BaseDetector):
    """
    XGBOD_uIForest is an unsupervised outlier detection algorithm that integrates the IForest
    ...
    def __init__(self, max_depth=3, learning_rate=0.1, n_estimators=100, objective="binary:logistic", booster='gbtree',
        ...
        """
        Initialize XGBOD_uIForest with hyperparameters for the XGBoost classifier.
        """
        super(XGBOD_uIForest, self).__init__()
    ...
    def _init_detectors(self, X):
        """
        Initialize the IForest detector.

        This method can be extended to support more detectors.
        """
        iforest = IForest()
        detectors = [iforest]
        standardization_flags = [False]  # IForest does not require standardization
        return detectors, standardization_flags

    def fit(self, X, y):
        """
        Fit the XGBOD_uIForest model to the training data X and labels y.
        """
        X, y = check_X_y(X, y)
        X = check_array(X)
    ...
    def _get_detector_scores(self, X):
        """
        Get the decision score from the IForest detector.
        """
